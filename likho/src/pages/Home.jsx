...
function Home() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-gray-800 text-white relative overflow-hidden px-4 md:px-12">
      {/* background blobs */}
      <motion.div className="absolute top-[-20%] left-[-10%] w-[400px] h-[400px] bg-teal-500 rounded-full mix-blend-multiply filter blur-3xl opacity-30"
      ...
      <div className={`text-center ${showWelcome ? "" : "pt-24"} mb-8 px-2`}>
        <motion.h2 className="font-extrabold text-2xl md:text-3xl text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-cyan-500" initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.8, ease: "easeOut" }}>
          Read Other Blogs
        </motion.h2>
        <motion.p className="mt-2 text-gray-400 text-sm md:text-base" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: "easeOut" }}>
